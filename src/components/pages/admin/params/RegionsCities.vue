    ...
    >
      ...
      >
        ...
        <CityComp 
          ...
          url="region"
          :city_id="el.id"
          @create="val => el.region.push(val)"
          @update="(val) => {
            el.region = el.region.map(item => 
              item.id === val.id 
                ? {...item, name: val.name} 
                : item
            )
          }"
          @del="val => el.region = el.region.filter((item) => item.id !== val)"
        />
      </q-card>
      <div class="q-ma-md">
...
import CityComp from 'src/components/pages/admin/params/CityComp'
...
const create = async () => {
  try { 
    const resp = await paramsApi.create(data.value)
